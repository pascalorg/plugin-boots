#!/usr/bin/env node
/**
 * BOOTS LISTEN QA HARNESS — mp-harness.mjs plus MODE=listen. Two pages, one
 * browser, no login.
 *
 *   MODE=listen    node scripts/qa/listen-harness.mjs
 *
 * A plays; B stays in the EDITOR and must HEAR A without ever being in the game:
 * a live recvonly link (`__bootsListen.debug()`), RTP bytes actually arriving
 * (`getStats`), the audio element holding a stream and not paused, `listen:true`
 * on B's voice frames, the listen pill reading LISTENING, A's pill naming the
 * listener, and getUserMedia never called on B. Then B clicks the spectator hint
 * and drops in: the SAME connection must survive the handover — no restart on
 * either side, A's m-line flips to sendrecv, and B's mic bytes reach A.
 *
 * AUDIBILITY IS NOT ASSERTED and cannot be here: both pages share one browser,
 * so voice.ts mutes the pair as SAME DEVICE (gain 0) to avoid feedback. Bytes,
 * directions and element state are the proof; `__bootsListen.localEcho(true)`
 * un-mutes for a human listening in HEADFUL=1.
 *
 * The other modes are inherited unchanged from mp-harness.mjs:
 *
...
if (!['play', 'spectate', 'voice', 'listen'].includes(MODE)) {
  console.error(`MODE must be play | spectate | voice | listen (got '${MODE}')`)
  process.exit(2)
}
/** B stays in the editor (spectate), and in 'listen' additionally proves it HEARS A, then drops in. */
const B_WATCHES = MODE === 'spectate' || MODE === 'listen'
...
async function main() {
  ...
  const urlB = B_WATCHES ? `${BASE}/scene/${SCENE}` : urlA
  log('goto', urlA, '|', urlB)
  await Promise.all([
  ...
  if (!B_WATCHES) await jumpIn(B, 'B', 'Bob')
  ...
  if (MODE === 'voice') {
    ...
    summary.listen = await proveListen(A, B)
  } else {
  ...
  if (!B_WATCHES && summary.framesBtoA === 0) summary.failures.push('A received no pose frames from B')
  ...
  if (!B_WATCHES && summary.avatarsSeenOnA < 1) summary.failures.push("no boots-remote-* avatar in A's scene")
  if (MODE === 'voice' && !summary.voice?.connectedBothWays) summary.failures.push('voice mesh not connected both ways')
  if (MODE === 'listen') {
    const l = summary.listen ?? {}
    if (!l.handle) summary.failures.push('spectator page never installed __bootsListen (listen effect did not start)')
    if (!l.recvonlyLink) summary.failures.push('spectator has no live recvonly link to A')
    if (!l.playerSendonly) summary.failures.push("A's side of the pair is not sendonly / not marked listener")
    if (!l.rtpBytes) summary.failures.push('no RTP bytes arrived on the spectator')
    if (!l.elementPlaying) summary.failures.push('spectator audio element has no stream or is paused')
    if (!l.listenFlagOnWire) summary.failures.push("spectator's voice frames do not carry listen:true")
    if (!l.pillListening) summary.failures.push(`listen pill did not read LISTENING (got '${l.pill}')`)
    if (!l.micNeverAsked) summary.failures.push('the spectator page asked for the microphone while listening')
    if (!l.playerPillNamesListener) summary.failures.push(`A's pill does not name the listener (got '${l.playerPill}')`)
    if (!l.handover?.ok) summary.failures.push(`handover failed: ${l.handover?.why ?? 'unknown'}`)
  }
}

/**
 * LISTEN: the spectator page (B, editor, never jumped in) must hold a live
 * RECVONLY link to A with RTP arriving and its audio element playing; A's side
 * must be sendonly and know B as a listener; B's frames must say listen:true;
 * B must never have asked for the mic. Then B drops in (the spectator hint pill)
 * and the SAME connection must survive: no restart on either side, A's side
 * flips to sendrecv, and B's mic audio starts arriving at A.
 *
 * Audibility is NOT asserted: both tabs share one browser, so voice.ts mutes the
 * pair as SAME DEVICE (gain 0). Bytes, directions and element state are the proof.
 */
async function proveListen(A, B) {
  const listenDebug = (page) => evalSafe(page, () => globalThis.__bootsListen?.debug?.() ?? null)
  const listenInternals = (page) => evalSafe(page, () => globalThis.__bootsListen?.internals?.() ?? null)
  const listenStats = (page) => evalSafe(page, async () => (await globalThis.__bootsListen?.stats?.()) ?? null)
  const listenPill = (page) => evalSafe(page, () => globalThis.__bootsListen?.pill?.() ?? null)
  const gameStats = (page) => evalSafe(page, async () => (await globalThis.__bootsVoice?.stats?.()) ?? null)
  const out = {
    handle: false,
    recvonlyLink: false,
    playerSendonly: false,
    rtpBytes: false,
    bytesReceived: 0,
    elementPlaying: false,
    listenFlagOnWire: false,
    pillListening: false,
    pill: null,
    micNeverAsked: false,
    B: null,
  ...
  // The mic-asked probe: getUserMedia calls on B, counted from the page.
  await evalSafe(B, () => {
    const md = navigator.mediaDevices
    if (!md || md.__bootsGumCount !== undefined) return
    md.__bootsGumCount = 0
  ...
  // 1. the listen effect starts once B sees a live player name (WATCH_POLL_MS)
  let deadline = Date.now() + 30000
  while (Date.now() < deadline) {
    if (await evalSafe(B, () => !!globalThis.__bootsListen)) {
      out.handle = true
      break
    }
    await B.waitForTimeout(500)
  }
  log(`listen: __bootsListen on B = ${out.handle}`)
  if (!out.handle) return out

  // 2. a connected link to A, RTP flowing
  deadline = Date.now() + 30000
  let dB = null
  let stB = null
  while (Date.now() < deadline) {
    dB = await listenDebug(B)
    const link = dB?.peers?.find((p) => p.sessionId === 'A-session')
    if (link && (link.state === 'connected' || link.connection === 'connected')) {
      stB = await listenStats(B)
      const row = stB?.find((s) => s.sessionId === 'A-session')
      if (row && row.bytesReceived > 0) break
    }
    await B.waitForTimeout(700)
  }
  const iB = await listenInternals(B)
  const iA = await voiceInternals(A)
  const dA = await voiceDump(A)
  const rowB = stB?.find((s) => s.sessionId === 'A-session')
  const rawB = iB?.find((e) => e.sessionId === 'A-session')
  const rawA = iA?.find((e) => e.sessionId === 'B-session')
  const linkA = dA?.peers?.find((p) => p.sessionId === 'B-session')
  const dirB = rawB?.transceivers?.map((t) => t.currentDirection ?? t.direction) ?? []
  const dirA = rawA?.transceivers?.map((t) => t.currentDirection ?? t.direction) ?? []
  out.recvonlyLink =
    !!dB?.listen &&
    !!dB?.peers?.some((p) => p.sessionId === 'A-session' && (p.state === 'connected' || p.connection === 'connected')) &&
    dirB.includes('recvonly')
  out.playerSendonly = !!linkA?.listener && dirA.includes('sendonly')
  out.bytesReceived = rowB?.bytesReceived ?? 0
  out.rtpBytes = out.bytesReceived > 0
  out.elementPlaying = !!rawB?.elementHasStream && rawB?.elementPaused === false
  const logB = (await busLog(B)) ?? []
  out.listenFlagOnWire = logB.some((e) => e.dir === 'out' && e.event === 'boots/voice' && e.data?.data?.listen === true)
  out.pill = await listenPill(B)
  out.pillListening = typeof out.pill === 'string' && /LISTENING/.test(out.pill)
  // The PLAYER's own pill must name the invisible audience: a listener has no
  // avatar and no presence, so this is the only thing that says it is there.
  out.playerPill = await evalSafe(A, () => globalThis.__bootsVoice?.pill?.() ?? null)
  out.playerPillNamesListener = typeof out.playerPill === 'string' && /LISTENING FROM THE EDITOR/.test(out.playerPill)
  out.micNeverAsked = (await evalSafe(B, () => navigator.mediaDevices?.__bootsGumCount ?? -1)) === 0 && dB?.mic === 'off'
  out.B = dB && { listen: dB.listen, mic: dB.mic, counters: dB.counters, peers: dB.peers.map((p) => ({ sessionId: p.sessionId, state: p.state, connection: p.connection, epoch: p.epoch, applied: p.applied, owed: p.owed, acked: p.acked, hasTrack: p.hasTrack, gain: p.gain, sameDevice: p.sameDevice })) }
  out.A = dA && { listen: dA.listen, mic: dA.mic, counters: dA.counters, peers: dA.peers.map((p) => ({ sessionId: p.sessionId, state: p.state, connection: p.connection, epoch: p.epoch, applied: p.applied, owed: p.owed, acked: p.acked, listener: p.listener, hasTrack: p.hasTrack })) }
  out.internalsB = rawB
  out.internalsA = rawA
  out.statsB = rowB
  log(
    `listen: B link ${out.recvonlyLink} (dir ${dirB.join(',')}) · A sendonly+listener ${out.playerSendonly} (dir ${dirA.join(',')}) · ` +
      `rx ${out.bytesReceived} B · element playing ${out.elementPlaying} · wire listen:true ${out.listenFlagOnWire} · pill '${out.pill}' · mic never asked ${out.micNeverAsked}`,
  )
  await B.bringToFront()
  await B.waitForTimeout(250)
  await B.screenshot({ path: `${SHOT_PREFIX}-B-listening.png` })
  summary.screenshots.push(`${SHOT_PREFIX}-B-listening.png`)

  // 3. HANDOVER: B drops in through the spectator hint pill; the link must survive.
  const restartsA0 = dA?.counters?.restarts ?? 0
  const restartsB0 = dB?.counters?.restarts ?? 0
  const pcMadeB0 = await evalSafe(B, () => globalThis.__bootsListen?.debug?.()?.peers?.length ?? 0)
  const hint = B.locator('[data-boots-spectator-hint]')
  const handover = { ok: false, why: null, clicked: false, inGame: false, sameLinkB: false, noRestartA: false, sendrecvA: false, bytesAtA: 0, listenOffB: false, B: null, A: null }
  try {
    await hint.first().waitFor({ state: 'visible', timeout: 10000 })
    await hint.first().click()
    handover.clicked = true
  } catch (e) {
    handover.why = `hint pill not clickable: ${String(e).slice(0, 120)}`
  }
  if (handover.clicked) {
    handover.inGame = await B.waitForFunction(() => !!globalThis.__boots, null, { timeout: 45000 }).then(() => true).catch(() => false)
    if (!handover.inGame) handover.why = 'B never entered the game after the hint click'
  }
  if (handover.inGame) {
    ...
    let stA = null
    while (Date.now() < deadline) {
      dB2 = await voiceDump(B)
      dA2 = await voiceDump(A)
      stA = await gameStats(A)
      const rowA = stA?.find((s) => s.sessionId === 'B-session')
      const linkB2 = dB2?.peers?.find((p) => p.sessionId === 'A-session')
      const linkA2 = dA2?.peers?.find((p) => p.sessionId === 'B-session')
      const iA2 = await voiceInternals(A)
      const dA2dir = iA2?.find((e) => e.sessionId === 'B-session')?.transceivers?.map((t) => t.currentDirection ?? t.direction) ?? []
      handover.listenOffB = dB2?.listen === false
      handover.sameLinkB = !!linkB2 && linkB2.state === 'connected' && (dB2?.counters?.restarts ?? 0) === restartsB0 && (dB2?.counters?.reaped ?? 0) === 0
      handover.noRestartA = !!linkA2 && linkA2.state === 'connected' && (dA2?.counters?.restarts ?? 0) === restartsA0 && linkA2.listener === false
      handover.sendrecvA = dA2dir.includes('sendrecv')
      handover.bytesAtA = rowA?.bytesReceived ?? 0
      if (handover.listenOffB && handover.sameLinkB && handover.noRestartA && handover.sendrecvA && handover.bytesAtA > 0) break
      await B.waitForTimeout(800)
    }
    handover.B = dB2 && { listen: dB2.listen, mic: dB2.mic, counters: dB2.counters, peers: dB2.peers.map((p) => ({ sessionId: p.sessionId, state: p.state, connection: p.connection, epoch: p.epoch, applied: p.applied, owed: p.owed, acked: p.acked, hasTrack: p.hasTrack })) }
    handover.A = dA2 && { mic: dA2.mic, counters: dA2.counters, peers: dA2.peers.map((p) => ({ sessionId: p.sessionId, state: p.state, connection: p.connection, epoch: p.epoch, applied: p.applied, owed: p.owed, acked: p.acked, listener: p.listener, hasTrack: p.hasTrack })) }
    handover.ok = handover.listenOffB && handover.sameLinkB && handover.noRestartA && handover.sendrecvA && handover.bytesAtA > 0
    if (!handover.ok) {
      handover.why = `listenOffB=${handover.listenOffB} sameLinkB=${handover.sameLinkB} noRestartA=${handover.noRestartA} sendrecvA=${handover.sendrecvA} bytesAtA=${handover.bytesAtA}`
    }
    await B.bringToFront()
    await B.waitForTimeout(250)
    await B.screenshot({ path: `${SHOT_PREFIX}-B-dropped-in.png` })
    summary.screenshots.push(`${SHOT_PREFIX}-B-dropped-in.png`)
  }
  handover.pcMadeB0 = pcMadeB0
  out.handover = handover
  log(`listen: handover ok=${handover.ok}${handover.why ? ` (${handover.why})` : ''} bytesAtA=${handover.bytesAtA}`)
  return out
}
