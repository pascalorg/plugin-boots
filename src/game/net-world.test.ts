...
describe('our own name is not a constant', () => {
  ...
  /**
   * THE OTHER DOOR INTO A RECORD LANE was `restorePending`, and a source fence
   * here used to forbid every caller of it: the mint fence above structurally
   * cannot see it (no `addLocal*` token anywhere), and `restoreLane` validated id
   * SHAPE only, so a natural "apply failed, put it back, retry" written against a
   * RECEIVED frame would have journaled a stranger's records and rotted this
   * lane's numbers — `staleMints` counting records `remintSharedRecords` can never
   * return, so `staleMints - staleReminted` stops being the loss figure Part 4
   * documents. That fence is gone because `restoreLane` now refuses records that
   * are not ours (`isOurs`, spanning former names), which makes the precondition
   * unnecessary instead of merely documented. The replacement is behavioural and
   * lives with the model: see "a restored frame keeps our records and refuses
   * another peer's" in shared-world.test.ts.
   */
