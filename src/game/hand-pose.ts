...
/** The knit cuff at the wrist: a step lighter than the sleeve so the wrist
 * reads as a wrist and not as more sleeve (first-person only for now). */
export const CUFF_HEX = '#454a56'

/** Palm slab: thickness (x), width across the knuckles (y), length wrist→knuckles (z),
 * and `x`, the inner face pressing the grip. */
export const PALM = { w: 0.072, t: 0.024, l: 0.08, x: 0.014 } as const
 ...
 * under the palm, squashed to `sx` of its radius across the hand's thickness
 * (x) — a wrist is an ellipse, not a ball. */
export const WRIST = { r0: 0.026, r1: 0.029, len: 0.035, z: 0.055, x: 0.024, sx: 0.62 } as const
...
export const FINGERS: readonly FingerSpec[] = [
  { name: 'index', y: 0.027, segs: [0.03, 0.024, 0.018], r: 0.0074, spread: 0.04 },
  { name: 'middle', y: 0.009, segs: [0.032, 0.026, 0.019], r: 0.0078, spread: 0.01 },
  { name: 'ring', y: -0.009, segs: [0.029, 0.024, 0.018], r: 0.0072, spread: -0.02 },
  { name: 'pinky', y: -0.027, segs: [0.022, 0.018, 0.015], r: 0.0062, spread: -0.06 },
]
...
/** Fingers taper: each segment's radius AT ITS JOINT as a share of the finger's
 * `r`; a segment runs from its own share down to the next one's (the last to
 * `FINGER_TIP`), so a finger is one smooth cone chain, not three sausages. */
export const FINGER_TAPER: readonly [number, number, number] = [1, 0.86, 0.74]
export const FINGER_TIP = 0.58
export const THUMB_TAPER: readonly [number, number] = [1, 0.86]
export const THUMB_TIP = 0.66
/** The knuckle (a finger's first joint, the thumb's root) is a touch fuller
 * than the finger it starts: the ridge you see across the back of a fist. */
export const KNUCKLE_BULGE = 1.1
...
/** One finger segment in the HAND frame: origin, XYZ Euler, length along its
 * local −Z, and its profile — `r` at the joint (origin), `r1` at the far end,
 * a joint sphere of radius `joint` at the origin (0 = none) and a tip sphere
 * of radius `tip` at the far end (0 = none; only the last segment of a chain
 * has one). `fingerSegmentTransforms` writes ACCUMULATED (flat) records;
...
export function makeSegment(): SegmentXform {
  return { px: 0, py: 0, pz: 0, rx: 0, ry: 0, rz: 0, len: 0, r: 0, r1: 0, joint: 0, tip: 0 }
}

/** Write a chain segment's profile: radius at the joint / far end, the joint
 * sphere (the first joint of a chain gets the knuckle bulge) and the tip. */
function segmentProfile(s: SegmentXform, base: number, share: number, nextShare: number, first: boolean, last: boolean): void {
  s.r = base * share
  s.r1 = base * nextShare
  s.joint = first ? s.r * KNUCKLE_BULGE : s.r
  s.tip = last ? s.r1 : 0
}
...
export function fingerSegmentTransforms(pose: HandPose, out: SegmentXform[]): SegmentXform[] {
  ...
  for (const f of FINGERS) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      s.rz = 0
      s.len = len
      segmentProfile(s, f.r, FINGER_TAPER[i] as number, i < 2 ? (FINGER_TAPER[i + 1] as number) : FINGER_TIP, i === 0, i === 2)
  ...
  for (let i = 0; i < 2; i++) {
    ...
    s.rz = 0
    s.len = len
    segmentProfile(s, THUMB.r, THUMB_TAPER[i] as number, i === 0 ? THUMB_TAPER[1] : THUMB_TIP, i === 0, i === 1)
...
export function indexChain(pose: HandPose): [SegmentXform, SegmentXform, SegmentXform] {
  ...
  segmentProfile(j0, f.r, FINGER_TAPER[0], FINGER_TAPER[1], true, false)
  const j1: SegmentXform = { px: 0, py: 0, pz: -f.segs[0], rx: 0, ry: curl[1], rz: 0, len: f.segs[1], r: 0, r1: 0, joint: 0, tip: 0 }
  segmentProfile(j1, f.r, FINGER_TAPER[1], FINGER_TAPER[2], false, false)
  const j2: SegmentXform = { px: 0, py: 0, pz: -f.segs[1], rx: 0, ry: curl[2], rz: 0, len: f.segs[2], r: 0, r1: 0, joint: 0, tip: 0 }
  segmentProfile(j2, f.r, FINGER_TAPER[2], FINGER_TIP, false, true)
  return [j0, j1, j2]
}
...
export function handBounds(pose: HandPose, withWrist = false): HandBounds {
  ...
  for (const s of segs) {
    grow(s.px, s.py, s.pz, s.joint > s.r ? s.joint : s.r)
    segmentEnd(s, end)
